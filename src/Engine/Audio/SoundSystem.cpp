...
namespace Audio
{
	...
	void SoundSystem::PlaySound(const std::string &soundName, int channel, bool loop, float Volume)
	{
	...
	void SoundSystem::PlaySoundSection(const std::string &soundName, int channel, bool loop, int StartMillseconds, int EndMilliseconds, float Volume)
	{
		auto PlayableSound = SoundSet.find(soundName);
		if (PlayableSound != SoundSet.end())
		{
			...
			if (StartMillseconds >= 0 || EndMilliseconds >= 0)
			{
				int startTime;
				int endTime;
				...
				{
					if (StartMillseconds > SongLength)
					{
#ifdef _DEBUG
						std::cerr << "Attempted to set song loop start point to longer then song length StartMS " << StartMillseconds << " | Song Length MS " << SongLength << std::endl;
#endif
					...
					{
#ifdef _DEBUG
						std::cerr << "Attempted to set song loop end point to longer then song length StartMS " << StartMillseconds << " | Song Length MS " << SongLength << std::endl;
#endif
						endTime = SongLength;
					}
					else
