...
namespace Mapping
{
	...
	bool TileMap::Walkable(int x, int y, int layerId)
	{
		...
		int index = 0;
		bool walkable = true;
		for(auto layer = collisionLayers.begin(); layer != collisionLayers.end(); ++layer, ++index)
		{
			if ((*layer)->IsActive())
			{
				if (layerId == -9000 || (*layer)->ActiveOnLayer(layerId))// || (*layer)->ActiveOnLayer(GetLayer(layerId)->GetName()))
				{
					walkable = walkable && !((*layer)->At(x, y) > 0);
				}
			}
		}
		return walkable;
	...
	void TileMap::Draw(sf::RenderWindow &window)
	{
		...
		if (layers.size() > 0)
		{
			...
			for (auto layer = layers.begin(); layer != layers.end(); ++layer, ++layerCount)
			{
				...
				/*if(charSprite.GetLayer() <= layerCount && !characterDrawn)
				{
					sf::View tempView(window.getView());
					window.setView(characterView);
					charSprite.draw(window, shader);
	...
	sf::Vector2f TileMap::SnapToTiles(sf::Vector2<int> &coordinates)
	{
		sf::Vector2<float> vec;
		vec.x = coordinates.x / tileWidth;
		vec.y = coordinates.x / tileHeight;
		return vec;
	}

	sf::Vector2f TileMap::SnapToTiles(sf::Vector2<float> &coordinates)
	{
		sf::Vector2<float> vec;
		vec.x = coordinates.x / tileWidth;
