...
#include <GL/glee.h>
		...
		void RocketSFMLRenderer::Resize()
		{
			...
			static sf::View View;
			View.SetSize(TargetWindow->GetWidth(), TargetWindow->GetHeight());
			...
			glMatrixMode(GL_PROJECTION);
			glLoadIdentity();
			glOrtho(0, TargetWindow->GetWidth(), TargetWindow->GetHeight(), 0, -1, 1);
...
#define BUFFER_OFFSET(x) ((char*)0 + x)
		...
		void RocketSFMLRenderer::SetScissorRegion(int x, int y, int width, int height)
		{
			...
			glScissor(x, TargetWindow->GetHeight() - (y + height), width, height);
		}
