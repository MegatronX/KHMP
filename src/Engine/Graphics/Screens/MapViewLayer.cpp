...
namespace Graphics
{
	namespace Screens
	{
		...
		void MapViewLayer::Draw(sf::RenderWindow &window)
		{
			...
			/*if (MapCharacter.FollowingInX() || MapCharacter.FollowingInY())
			{
				restoreView = true;
				sf::Vector2f NewCenter = prevView.getCenter();
				if (MapCharacter.FollowingInX())
					NewCenter.x = MapCharacter.GetMapPosition().x;
				if (MapCharacter.FollowingInY())
					NewCenter.y = MapCharacter.GetMapPosition().y;
				CharacterView.setCenter(NewCenter);
		...
		void MapViewLayer::Draw(sf::RenderWindow &window, sf::Shader &shader)
		{
			...
			//CharacterView.setCenter(MapCharacter.GetMapPosition());
			//window.SetView(CharacterView);
			/*if (MapCharacter.FollowingInX() || MapCharacter.FollowingInY())
			{
			restoreView = true;
