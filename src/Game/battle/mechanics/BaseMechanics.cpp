...
namespace Game
{
	namespace Battle
	{
		...
		void BaseMechanics::ProcessAction(Actions::Action& action)
		{
			...
			{
				auto SEAbils = GetEffectsWithCallCondition(PreUse, action.Users, !action.IgnoreUserSE, !action.IgnoreUserAbil);
				auto SEAbilDef = GetEffectsWithCallCondition(PreUse, action.Targets, !action.IgnoreTargetSE, !action.IgnoreTargetAbil);
				bool cont = true;
				for (auto it = SEAbils.begin(); it != SEAbils.end() && cont; ++it)
				{
					auto puseCmp = (*it)->GetComponentAs<Action::ActionModifierComponent*>("PreUse");
					if (puseCmp != nullptr)
					{
				...
				SEAbils = GetEffectsWithCallCondition(OutgoingAttack, action.Users);
				cont = true;
				for (auto it = SEAbils.begin(); it != SEAbils.end() && cont; ++it)
				{
					auto puseCmp = (*it)->GetComponentAs<Action::ActionModifierComponent*>("OutgoingAttack");
					if (puseCmp != nullptr)
					{
						cont = puseCmp->ModifyAction(action, *this);
					}
				...
				if (action.Users.size() > 1)
				{
					...
					for (auto it = action.Users.begin(); it != action.Users.end(); ++it)
					{
						auto btlChr = (*it).AttachedCharacter->GetComponentAs<BattleCharacterComponent*>("BattleCharacterComponent");
						if (btlChr != nullptr)
						{
							auto party = btlChr->GetBaseParty();
							auto vec = party->GetPartyMembers();
				...
				{
					auto btlChr = (*action.Users.begin()).AttachedCharacter->GetComponentAs<BattleCharacterComponent*>("BattleCharacterComponent");
					if (btlChr != nullptr)
					{
						auto party = btlChr->GetBaseParty();
						auto vec = party->GetPartyMembers();
				...
				for (auto it = SEAbils.begin(); it != SEAbils.end() && cont; ++it)
				{
					auto puseCmp = (*it)->GetComponentAs<Action::ActionModifierComponent*>("PartyMemberHadOutgoingAttack");
					if (puseCmp != nullptr)
					{
						cont = puseCmp->ModifyAction(action, *this);
					}
			...
			auto SEAbils = GetEffectsWithCallCondition(FinalizingTargets, action.Users, !action.IgnoreUserSE, !action.IgnoreUserAbil);
			auto SEAbilDef = GetEffectsWithCallCondition(FinalizingTargets, action.Targets, !action.IgnoreTargetSE, !action.IgnoreTargetAbil);
			bool cont = true;
			for (auto it = SEAbils.begin(); it != SEAbils.end() && cont; ++it)
			{
				auto puseCmp = (*it)->GetComponentAs<Action::ActionModifierComponent*>("FinalizingTargets");
				if (puseCmp != nullptr)
				{
					cont = puseCmp->ModifyAction(action, *this);
				}
		...
		std::vector<StatusEffects::StatusEffect*> BaseMechanics::GetEffectsWithCallCondition(CallCondition cc, std::vector<Character::BaseCharacter*>& characters, bool IncludeSE, bool IncludeAbil)
		{
			...
			for(auto it = characters.begin(); it != characters.end(); ++it)
			{
				...
				if (IncludeSE)
				{
					auto SEMng = (*it)->GetComponentAs<StatusEffects::StatusEffectsManager*>("StatusEffectManager");
					if (SEMng != nullptr)
					{
		...
		std::vector<StatusEffects::StatusEffect*> BaseMechanics::GetEffectsWithCallCondition(CallCondition cc, std::vector<Actions::ActiveAction>& characters, bool IncludeSE, bool IncludeAbil)
		{
			std::vector<StatusEffects::StatusEffect*> ineffect;
			ineffect.reserve(characters.size() * 2);
			for(auto it = characters.begin(); it != characters.end(); ++it)
		...
		int BaseMechanics::GenerateInt(const int min, const int max, float bias)
		{
			/*if (OwningField != nullptr && !RandomDisabled)
			{
				return OwningField->GetRandomInt(min, max);
			}
			else*/
			{
		...
		float BaseMechanics::GenerateFloat(const float min, const float max, float bias)
		{
			/*if (OwningField != nullptr && !RandomDisabled)
			{
				return OwningField->GetRandomFloat(min, max);
			}
			else*/
			{
				return (min + max) * 0.5f;
