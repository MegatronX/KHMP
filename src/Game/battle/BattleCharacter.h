...
namespace Game
{
	namespace Battle
	{
		...
		typedef boost::signals2::signal<void (BattleCharacter* bchar, bool IsSim = false)> TimerOverflowSignal;
		class BattleCharacter : public Component
		{
		public:
			BattleCharacter(Character::BaseCharacter* owner) : Component(owner, owner != nullptr ? owner->GetName() + "BattleCharacterComponent" : "NoOwnerBattleCharacterComponent", "BattleCharacterComponent").
				TimerValue(0), SimeTimerValue(0), OverflowRatio(0.f), CanTakeTurn(true), InSimulation(false), sm(owner != nullptr ? owner->GetComponentAs<Character::StatManager*>("StatManagerComponent") : nullptr)
			{
			...
			void TickTimer(int ticks = 1)
			{
				...
				if (InSimulation)
				{
					SimTimerValue += sm->GetBattleStat(Stat::Speed);
				}
				else
				{
					LiveTimerValue += sm->GetBattleStat(Stat::Speed);
				}
#ifdef DEBUG
				}
				else
