...
namespace Game
{
	namespace Scripting
	{
		...
		struct ActionModifyWrap : public Actions::ActionModifierComponent
		{
			ActionModifyWrap(PyObject* self_) : self(self_)
			{

			}
			ActionModifyWrap(PyObject* self, const Actions::ActionModifierComponent& ref) : self(self), ActionModifierComponent(ref)
			{

			}
			ActionModifyWrap(PyObject* self_, Entity* owner) : ActionModifierComponent(owner), self(self_)
			{

			}
			ActionModifyWrap(PyObject* self_, Entity* owner, const std::string& name) : ActionModifierComponent(owner, name), self(self_)
			{
			...
			{
				return this->ActionModifierComponent::ModifyAction(action, mechanics);
			}
			PyObject* self;
		};
		...
		BOOST_PYTHON_MODULE(ActionModule)
		{
			...
			class_<Actions::Action, boost::shared_ptr<ActionWrap>, bases<Entity> >("Action")
				...
				;
			class_<Actions::ActionModifierComponent, boost::shared_ptr<ActionModifyWrap>, bases<Components::Component> >("ActionModifierComponent")
				.def(init<>())
				.def(init<const Actions::ActionModifierComponent&>())
				.def(init<Entity*>())
				.def(init<Entity*, const std::string&>())
				.def("ModifyAction", &Actions::ActionModifierComponent::ModifyAction, &ActionModifyWrap::ModifyAction)
				;
