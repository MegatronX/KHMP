...
namespace Input
{
	extern const boost::unordered_map<InputAction, std::string> ActionNameLookup = boost::assign::map_list_of
		...
		(InputAction::Up, "Up");
	extern const boost::unordered_map<std::string, InputAction> NameActionLookup = boost::assign::map_list_of
		...
		("Up", InputAction::Up);
	extern const boost::unordered_map<std::string, sf::Keyboard::Key> StringToKeyMap = boost::assign::map_list_of
		("A", sf::Keyboard::A)
		("B", sf::Keyboard::B)
		("C", sf::Keyboard::C)
		("D", sf::Keyboard::D)
		...
		;
	extern const boost::unordered_map<sf::Keyboard::Key, std::string> KeyToStringMap = boost::assign::map_list_of
		(sf::Keyboard::A, "A")
		(sf::Keyboard::B, "B")
		(sf::Keyboard::C, "C")
		(sf::Keyboard::D, "D")
	...
	InputModule::InputModule(const std::string& configFile, Engine* _engine) : engine(_engine)
	{
	...
	bool InputModule::IsActionPushed(PlayerInput input, const InputAction action) const
	{
		PlayerInputSet* set = nullptr;
		switch (input)
		{
		case P1Input:
			set = const_cast<PlayerInputSet*>(&Player1InputMap);
			break;
#if MaxPlayerCount > 1
		case P2Input:
			set = const_cast<PlayerInputSet*>(&Player2InputMap);
			break;
#endif
#if MaxPlayerCount > 2
		case P3Input:
			set = const_cast<PlayerInputSet*>(&Player3InputMap);
			break;
#endif
