...
namespace Content
{
	...
	ContentManager::ContentManager(sf::Uint32 StartTime, sf::Uint32 ResolutionTime, sf::Uint32 ImageLifeTime, sf::Uint32 FontLifeTime, sf::Uint32 AudioLifeTime) 
		: UpdateRate(ResolutionTime), TimeUpdateRate(sf::milliseconds(ResolutionTime)), RunningTime(StartTime), PrimaryFontCache(200, FontLifeTime, 5), UpdaterThread(boost::bind(&ContentManager::ThreadUpdater, this)) //: PrimaryImageCache(200, ImageLifeTime, 5)
	{
...
/*	Graphics::animsprite_ptr ContentManager::RequestSprite(const std::string &name, const Cache::GameStyle style, const Cache::World world, const Cache::ResourceClass rClass, const sf::Uint32 time, const bool Start, const std::string extension)
	...
	Graphics::animspr_ptr ContentManager::RequestAnimatedSprite(const std::string& name, const Cache::GameStyle style, const Cache::World world, const Cache::ResourceClass rClass, const sf::Uint32 time, const bool Start, const std::string extension)
	{
		auto img = PrimaryTextureCache.RequestResource(name, style, world, rClass, time, -1, true, extension);
		auto it = StoredAnimators.find(name);
		if (it != StoredAnimators.end())
		{
			return Graphics::animspr_ptr(new ::Graphics::AnimatedSprite(img, it->second.get()));
		}
		return Graphics::animspr_ptr();
		//auto texture = Pri
	}
	...
	void ContentManager::Initialize(Configuration::AppConfiguration &configuration)
	{
		...
		MapDirectory = configuration.GetApplicationSetting("ResourcesBaseDirectory") + configuration.GetApplicationSetting("MapSubDir");
	    ShaderDirectory = configuration.GetApplicationSetting("ResourcesBaseDirectory") + configuration.GetApplicationSetting("ShaderSubDir");
		HTMLDirectory = configuration.GetApplicationSetting("HTMLDirectory");
		ResourceLoadImport::ProcessImports(configuration.GetApplicationSetting("ResourcesBaseDirectory") + std::string("LoadDescriptions.xml"), this);
	}
