...
#include <GL/glee.h>
		...
		RocketSFMLRenderer::RocketSFMLRenderer() : target(nullptr), states(sf::BlendMode::BlendNone)
		{
		}
		...
		void RocketSFMLRenderer::SetWindow(sf::RenderTarget *Window)
		{
			target = Window;
		...
		sf::RenderTarget *RocketSFMLRenderer::GetWindow()
		{
			return target;
		};
		...
		void RocketSFMLRenderer::Resize()
		{
			...
			View.setCenter(target->getView().getCenter());
