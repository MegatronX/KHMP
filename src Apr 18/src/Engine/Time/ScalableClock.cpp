...
ScalableClock::ScalableClock(const float scale, sf::Uint32 resolution) 
	: TimeScaler(scale), Resolution(resolution), TimeResolution(sf::milliseconds(resolution)), TimeBuffer(0), Scaled(!(scale >= 0.99 && scale <= 1.01)), UpdaterThread(boost::bind(&ScalableClock::UpdateFunction, this))
{
}
